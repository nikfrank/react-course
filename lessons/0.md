...
- if we're hitting, our new cards array will be the old array with a ```newCard()``` appended (newCard is a function declared earlier in the file which generates an object with a random ```rank``` and ```suit``` property, each being just a number)
