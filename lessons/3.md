...
- [evaluated keys](http://www.benmvp.com/learning-es6-enhanced-object-literals/#computed-property-keys)
...
here we're using ```setState```'s [update function param](https://reactjs.org/docs/react-component.html#setstate) instead of just passing an object. This is the original use case that the "updater function" was made for.
...
render(){
  ...
  //... (put this inside the shopping-list div as well!)
  <ul>
    {
...
if the index (sli := 'shopping list index') isn't the same as the index we computed from the ```id``` prop param of our onChange event, the ```map``` function returns the old item in place
